...
public class TakeSubCommand extends ChangeMoneySubCommand {
    ...
    protected boolean tryChange(CommandSender sender, OfflinePlayer offlinePlayer, double amount) {
        boolean success = instance.get(EconomyHandlerProvider.class).getEconomyHandler().withdraw(Utils.getUniqueId(offlinePlayer), amount);
        if (success && sender instanceof Player) {
            UUID senderUUID = ((Player) sender).getUniqueId();
            TransactionEvent event = new TransactionEvent(senderUUID, Utils.getUniqueId(offlinePlayer), amount, TransactionEvent.TransactionType.TAKE);
